...
### client-service-period-invalid

`CLIENT_SERVICE_PERIOD_INVALID`

### client-service-period-uncomputable

`CLIENT_SERVICE_PERIOD_UNCOMPUTABLE`

### client-duration-out-of-range

`CLIENT_DURATION_OUT_OF_RANGE`

### client-duration-needs-service-period

`CLIENT_DURATION_NEEDS_SERVICE_PERIOD`

### client-service-status-invalid

`CLIENT_SERVICE_STATUS_INVALID`

### client-area-unavailable
...
### employee-phone-already-registered

`EMPLOYEE_PHONE_ALREADY_REGISTERED`

### employee-active-assignment-blocked

`EMPLOYEE_ACTIVE_ASSIGNMENT_BLOCKED`

### employee-assignment-not-eligible

`EMPLOYEE_ASSIGNMENT_NOT_ELIGIBLE`

### service-record-write-target-changed

`SERVICE_RECORD_WRITE_TARGET_CHANGED`

### employee-schedule-overlap

`EMPLOYEE_SCHEDULE_OVERLAP`

### schedule-retention-blocked

`SCHEDULE_RETENTION_BLOCKED`

### service-record-planned-date-unavailable
