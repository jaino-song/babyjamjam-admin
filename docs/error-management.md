...
- 독립 검토에서 문자 거절 뒤 예약 재시도, 일부 접수 뒤 전체 수신자 재발송, 재시도 응답의 느슨한 성공 판정이 발견됐다. 이 세 항목의 수정 및 독립 재검수가 끝나기 전에는 문자 복구 경로 완료를 선언하지 않는다.
- 계약 작업·문자 작성 화면·로컬 요청 검증 경계의 추가 전환은 통합 검증 중이다. UI 잠금은 현재 화면 세션에만 적용되며 서버의 영속적인 멱등성 보장으로 해석하지 않는다.
