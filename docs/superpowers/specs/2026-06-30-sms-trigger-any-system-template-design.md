...
- **`triggerType` — fixed, event-agnostic string per template** (resolved during planning, supersedes the earlier "derive from event" idea). The job entity carries **no** `eventType` (only `ruleId`), and `triggerType` is informational only — it lives inside the log's `variables` JSON blob, not a column, and is not used for retry/dedup. So each of the 5 new configs gets a fixed string (`price_info`, `reminder`, `thanks`, `survey`, `info`); the 2 legacy templates keep their existing strings byte-for-byte (`service_start_before_7_days`, `client_created`) → history unchanged. `triggerType` stays **required** on the config (no type change, no eventType plumbing).
