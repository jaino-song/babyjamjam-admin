...
Task2.2 수치 검증 감사 보정: 공통 양식 descriptor의 수치 하위 항목 metadata로 UI와 같은 min/step을 검증한다. 횟수/양 정수 항목은0이상 유한 정수, 체온은0이상 유한0.1단위를 허용하며 기존 유효 숫자 문자열과 부분 초안의 빈 문자열은 유지한다. 임의 문자열/음수/잘못된 소수/비유한 값은 관리자 초안·제공인력 API 모두 저장 전에 거부한다. `packages/shared/src/constants/service-record-form-layout.ts`, 기존 runtime 생성물, policy·admin/public 테스트는 이 좁은 보정의 추가 소유 경로다. UI 표시/옵션 또는 실제 임상 범위를 새로 정의하지 않는다. `backend/test/e2e/full-flow.e2e.spec.ts`의 오래된 stool 선택값 두 곳만 현재 정식 UI 값으로 보정하고 같은 payload의 단위 회귀를 검증한다. 이 기존 넓은 모듈/DB harness를 운영 환경값으로 실행하지 않으며 실제 HTTP+DB 시험은 Phase6의 별도 guard runner를 사용한다.
...
- **Task 3.0: 공통 잠금 순서 확정과 회귀 검증** (refactor, high)
  ...
  **추가 owning transaction Paths:** `backend/application/usecases/client/update-client.usecase.ts`, `backend/domain/repositories/client.repository.interface.ts`, `backend/infrastructure/database/repositories/sb.client.repository.ts` 및 관련 테스트. 서비스 중단/교체 완료의 기존 고객 수정과 lifecycle 동기화를 동일한 typed transaction으로 전달한다. 중첩 root transaction이나 업무 로직 복제는 하지 않으며 이 task에서 duration/가격/N 의미는 변경하지 않는다.
  **추가 삭제 owning Paths:** `backend/application/usecases/employee-schedule/delete-employee-schedule.usecase.ts`, `backend/domain/repositories/employee-schedule.repository.interface.ts`, 기존 허용 `backend/infrastructure/database/repositories/sb.employee-schedule.repository.ts` 및 관련 테스트. 배정 삭제와 후속 lifecycle 동기화도 같은 트랜잭션으로 처리한다. 기존 삭제 정책을 바꾸지 않고 잠금 뒤 대상 소유권을 재검사한다.
...
병렬 실행 보정: 사용자의 단계별 병렬 실행 지시에 따라 Task5.1/5.3/5.4/5.2의 내부 구현은 같은 배치에서 시작하며, 공용 저장소·타입 체크포인트를 받은 뒤 연결한다. 아래 Depends는 최종 연결 및 수락 의존성이다. 정확한 서로소 파일 소유와 병렬 배치는 `phase5-execution-contract.md`를 따른다. Phase4 감사와 Phase5 통합 감사 게이트는 유지한다.
...
| 직원 2명/교체 이력/기존 다른 고객 배정 | 기존 귀속 유지, 영향 배정 범위 계산. 같은 고객의 귀속 역전·모호함은 차단하고 기존 정책이 허용하는 다른 고객의 일정 겹침은 유지하며 그 행은 변경하지 않음 |
