...
실행 주석: 사용자의 2026-09-07 실행 지시에 따라 각 구현 task는 gpt-5.6-luna/max로 수행한다. local은 luna_implementer, 승인된 network는 luna_network_implementer를 사용한다. 메인 에이전트는 통합과 검증을 담당한다. 독립 검토자는 지정된 Sol 역할을 사용한다. 구현 작업은 순차 실행하며 병렬 배치는 없다.
...
- **Task 6.2: 독립 최종 검토** (test, high)
  ...
  **Tier:** standard · **Sandbox:** local · **Agent:** sol_reviewer · **Model:** gpt-5.6-sol · **Effort:** high  
