...
Receipt invariant mapping: `backend/test/e2e/service-record-receipt-promotion.e2e.spec.ts` — `promotes only the artifact while preserving token identity, contract FK, and receipt values` explicitly asserts unchanged receivedDate/amount; `preserves revision-era token state on public lookup while retaining legacy restoration` verifies the existing link access path; `fails closed for stale proof, state version, and revision without replacing the old artifact`, `allows exactly one concurrent promotion for a generation and retains that winner`, and `preserves expired and revoked tokens without issuing a token or SMS` cover stale/concurrent/access constraints. These six actual PG cases passed in Phase5's final matrix.

Phase6 actual HTTP initial run (`/tmp/bjj-phase6-http-red.log`): 14PASS/3FAIL. Two failures were the real missing query rejection (forgedbranch got200/201); one was a fixture assertion passing undefined into a helper with environment defaults. Commit `24e85c048` changes the guard fixture to explicit empty strings; `9a6703b16` pins the runner's synthetic JWT signing secret. Neither changes the production authentication implementation. Query correction is still awaiting integrated actual HTTP rerun. Random synthetic auth/client rows remain only in the owned disposable DB; no global auth tables are cleared.
...
- Parent visual inspection confirmed all13 date cards, existing480px layout,14fields and original/revised values. It also caught visually adjacent date labels (`newdate원본olddate`), so a narrow existing date-display spacing correction is pending before final audit.
