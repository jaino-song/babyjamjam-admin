...
## Phase 3a — 고객 생성/수정 사전 검증 오류의 구조화 (바인딩, 2026-09-10)

TL;DR: 고객 저장 전 입력 검증 9개 지점(연락처 형식, 날짜 형식, 기간 순서·계산, 기간 범위, 상태 enum, 지역, 수정 시 필수 필드 비움)을 `VALIDATION_FAILED` 문제 본문 + `errors[]`(pointer/code/detail)로 바꿔 기존 고객 폼이 필드 오류와 구체 안내를 그대로 표시하게 한다. 중복 연락처·자동 등록 설정·404·P2002 경쟁은 별도 단위(3.1b/3.1c)로 분리한다.

- **Task 3.1a: 고객 write 사전 검증의 공개 계약 전환** (feature, high)
  - `client-write-validation.ts`의 7개 함수를 `BadRequestException(problemBody)`로 전환한다: `assertClientPhoneInput`→`/phone` INVALID_FORMAT, `parseClientDate(value, field)`→`/<field>` INVALID_FORMAT(23개 호출부에 field 전달), `deriveClientDuration`→`/endDate` INVALID_VALUE(단, `UnsupportedKoreanHolidayYearError`의 영문 기술 메시지 노출을 "서비스 기간을 계산할 수 없습니다."로 대체), `assertClientDurationMatchesDates`→`/duration` OUT_OF_RANGE, `mergeAndValidateClientServicePeriod`→`/endDate` INVALID_VALUE, `assertAllowedServiceStatus`→`/serviceStatus` INVALID_VALUE, `assertAllowedClientArea`→`/areaId` INVALID_VALUE. `client.service.ts` update의 직접 throw 3곳(필수 필드 비움 `/name|/voucherClient|/breastPump` REQUIRED, duration 2곳 `/duration`)도 같은 헬퍼로 전환한다.
  - 제외(후속 단위): `create`의 자동 등록 비활성 409(L1036), 중복 연락처(clientId 포함, L1050·L1639·`assertPhoneAvailable`), update 대상 404(L1590), BFF·UI 변경, 공유 카탈로그 신규 코드. 조건·검증 순서·성공 경로는 그대로 유지한다.
  - 검증: 기존 spec 3파일의 메시지 단언을 구조화 단언으로 갱신하고, 대표 create/update 오류가 `mapHttpProblem→sendProblemResponse→normalizeApiError`를 지나 `verified:true`·`errors[]` 보존·NONE/NEVER·민감정보 비노출임을 확인한다. red-first로 전환 전 실패를 기록한다.
  - Dispatch metadata: `Phase: 3a` · `Parallel group: none` · `Execution: DELEGATE` · `Audit: SOL` · `Decision reason: 다중 파일·테스트 전환량과 공개 응답 의미 변경; 자기완결 brief 가능` · `Tier: standard` · `Sandbox: local` · `Agent: worker` · `Model: opencode-go/glm-5.3-flash` · `Effort: default (no variant override)` · `Phase starting integration commit: 40531bdd434c7182d9b731885f880b90fc28da79` · `Integration worktree: /Users/jaino/Development/babyjamjam-admin/korean-error-messages` · `Branch: unit/bjj319-client-write-validation` · `Worktree: /Users/jaino/Development/babyjamjam-admin/unit-bjj319-client-write-validation` · `Service tier: default` · `Paths: backend/application/usecases/client/client-write-validation.ts, backend/application/services/client.service.ts [create/update 사전 검증 throw만], backend/application/usecases/client/client-write-agent-capabilities.provider.ts [parseClientDate 호출부만], backend/application/usecases/client/client-write-validation.spec.ts, backend/test/services/client.service.spec.ts, backend/application/usecases/client/client-write-agent-capabilities.provider.spec.ts, backend/test/integration/client.controller.integration.spec.ts [필요한 좁은 항목만]` · `Depends: Task 1.1`
  - **Phase close gate:** unit 검증 → 통합 → 영향 통합 검증(대상 spec + 타입 + lint + diff check) → exact SHA/base `auditor` FINAL SHIP → inventory의 `client-write-validation.ts`·`client.service.ts`(사전 검증 부분) 증거 갱신과 본 계획 기록 → clean unit worktree/branch 정리. 3.1a가 끝나도 Task 3.1 전체(중복 연락처·수정 충돌·삭제 제한)와 전체 이슈는 미완료다.
