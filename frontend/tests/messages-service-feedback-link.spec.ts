...
const FEEDBACK_URL = "https://mobile.test/feedback/efl_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
const PREPARED_LINK_TOKEN = FEEDBACK_URL.split("/").at(-1)!;

const serviceFeedbackTemplate = `송진호 관리사님, 송진호 산모님의 서비스 제공기록지 작성 링크입니다.
...
test("shows the exact prepared feedback URL and sends the same token", async ({ page }) => {
    ...
    await page.route("**/api/**", async (route: Route) => {
        ...
        if (pathname === "/api/system-templates/SERVICE_FEEDBACK_LINK") {
            return route.fulfill({
                status: 200,
                contentType: "application/json",
                body: JSON.stringify({
                    key: "SERVICE_FEEDBACK_LINK",
                    content: serviceFeedbackTemplate,
    ...
    await expect(messageField).toHaveValue(new RegExp(FEEDBACK_URL));
    await expect(messageField).not.toHaveValue(/\{\{feedbackUrl\}\}/);
