...
/**
 * Mirrors the mobile read-query contract (mobile/src/hooks/__tests__/usePushNotification.test.tsx):
 * a failed or malformed read must surface as a query error, never as an empty
 * success (EM-STATE-01 failure→empty-success ban; docs/error-management.md).
 */
describe("notification read queries", () => {
  ...
  it("keeps a valid notification list as a successful read", async () => {
    mockedApiGet.mockResolvedValue({ data: [{ id: 1 }] });
