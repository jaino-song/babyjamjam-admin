...
describe("POST /api/eformsign-docs/refresh-token", () => {
    it("returns a deterministic 410 problem without exposing credentials", async () => {
        const response = await POST(
            new NextRequest("http://localhost/api/eformsign-docs/refresh-token", {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ refreshToken: "secret-refresh" }),
            }),
        );
