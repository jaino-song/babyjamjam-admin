...
describe("GET /api/eformsign-docs/review-needed-contracts", () => {
    beforeEach(() => {
        mockGet.mockReset();
    });
    ...
    it("rejects an unauthenticated read with a registered 401 problem body", async () => {
        const response = await GET(
            new NextRequest("http://localhost/api/eformsign-docs/review-needed-contracts"),
        );
    ...
    it("sanitizes a legacy upstream failure instead of a raw English body", async () => {
        mockGet.mockRejectedValue({
            response: { status: 500, data: { message: "review queue shard-9 exploded" } },
        });
