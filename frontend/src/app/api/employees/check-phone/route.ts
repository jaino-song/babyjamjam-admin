...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await serverAPIClient.get<CheckPhoneResponse>("/employees/check-phone", {
    ...
    // 업스트림 실패는 `exists: false`로 위장하지 않는다 — 중복 확인
    // 실패/재시도 UI가 동작하도록 sanitizer로 전달한다.
