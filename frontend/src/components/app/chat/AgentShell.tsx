...
export function AgentShell() {
    ...
    const { messages, sendMessage, status, error, actionError, taskError, taskSnapshotState, taskAccessState, taskMutationInFlight, taskNeedsReconciliation, createTaskEventId, commandTask, loadTaskSnapshot, stop, regenerate, resetBranch, sessions, selectSession, renameSession, deleteSession, approveAction, rejectAction, submitStructuredForm, submitFeedback } = useAgentChat();
        ...
        <main data-component="desktop_chat_agent-shell" data-source-component="AgentShell" className="fixed inset-0 z-[1200] grid grid-cols-[auto_1fr] bg-background">
            ...
            <section data-component="desktop_chat_agent-shell_thread" className="flex min-w-0 flex-col">
                ...
                <ScrollArea className="min-h-0 flex-1"><div data-component="desktop_chat_agent-shell_thread_messages" className="mx-auto flex w-full max-w-3xl flex-col gap-6 p-4 sm:p-8">
                    ...
                    {taskError && <ErrorPart data-component="desktop_chat_agent-shell_thread_messages_task-error" code={taskError.code} category="persistence" message={taskError.message} retryable={retryTaskId !== undefined} effectState="nothing-happened" onRetry={retryTaskId && loadTaskSnapshot ? () => void loadTaskSnapshot(retryTaskId) : undefined} />}
                </div></ScrollArea>
