...
describe("client registration formats", () => {
    ...
    it("accepts a leap-day birthday only when the resolved year has one", () => {
        expect(getCanonicalClientRegistrationError({ ...validInput, birthday: "2000-02-29" })).toBeNull();
        expect(getCanonicalClientRegistrationError({ ...validInput, birthday: "1999-02-29" })).toContain("생년월일");
        expect(getCanonicalClientRegistrationError({ ...validInput, birthday: "2000-02-30" })).toContain("생년월일");
    });
    ...
    it("rejects invalid calendar dates and short phone numbers before submit", () => {
        ...
        expect(getCanonicalClientRegistrationError({ ...validInput, birthday: "1990-02-30" })).toContain("생년월일");
