...
const ACTION_SKELETON =
  '[data-component="mobile_messages_new_page_screen_form_scroll_list-card_header_action-skeleton"]';
const ACTION_BUTTON =
  '[data-component="mobile_messages_new_page_screen_form_scroll_list-card_header_action"]';
...
test.describe("Mobile new-message permission skeletons", () => {
  ...
  test("keeps the form up and skeletons the permission-dependent controls while approval loads", async ({
    ...
    // The "즉시 발송" action is the shared permission skeleton, not the button.
    await expect(page.locator(ACTION_SKELETON)).toBeVisible();
    await expect(page.locator(ACTION_BUTTON)).toHaveCount(0);
