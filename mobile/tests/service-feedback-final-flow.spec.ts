...
test("백엔드 서비스 일수만큼 제공기록표를 생성한다", async ({ page }) => {
  ...
  await page.route(`**/api/feedback/${token}/link`, (route) => route.fulfill({ json: { valid: true } }));
  await page.route(`**/api/feedback/${token}/verify`, (route) => route.fulfill({ status: 201, json: { ok: true, accessToken: "access-token" } }));
  await page.route(`**/api/feedback/${token}/context`, (route) => route.fulfill({
    json: {
  ...
  await expect(page.locator('[data-component="feedback-day-grid"] > button')).toHaveCount(20);
  await expect(page.locator('[data-component="feedback-day-number"]').filter({ hasText: "20" })).toHaveCount(1);
});
...
test("마지막 회차 제출 후 별도 버튼 없이 최종 제출을 완료한다", async ({ page }) => {
  ...
  await page.route(`**/api/feedback/${token}/link`, (route) => route.fulfill({ json: { valid: true } }));
  await page.route(`**/api/feedback/${token}/verify`, (route) => route.fulfill({ status: 201, json: { ok: true, accessToken: "access-token" } }));
  await page.route(`**/api/feedback/${token}/header`, async (route) => {
    headerSaved = true;
    await route.fulfill({ json: { ok: true } });
  });
  await page.route(`**/api/feedback/${token}/sessions/1/submit`, async (route) => {
    submittedBody = route.request().postDataJSON() as Record<string, unknown>;
    submitted = true;
    await route.fulfill({ status: 201, json: { ok: true } });
  });
  await page.route(`**/api/feedback/${token}/finalize`, async (route) => {
    finalizeCalls += 1;
    await route.fulfill({ json: { ok: true } });
  });
  await page.route(`**/api/feedback/${token}/context`, (route) => route.fulfill({
  ...
  const breastFeedingField = page.locator('[data-component="feedback-day-field"]').filter({ hasText: "⑧ 모유수유" });
  const formulaFeedingField = page.locator('[data-component="feedback-day-field"]').filter({ hasText: "⑨ 분유수유" });
...
test("기본정보 저장 실패 시 입력값을 보존하고 다음 단계로 이동하지 않는다", async ({ page }) => {
  ...
  await mockShellRequests(page);
  await page.route(`**/api/feedback/${token}/link`, (route) => route.fulfill({ json: { valid: true } }));
  await page.route(`**/api/feedback/${token}/verify`, (route) => route.fulfill({
    status: 201,
    json: { ok: true, accessToken: "access-token" },
  }));
  await page.route(`**/api/feedback/${token}/header`, (route) => route.fulfill({
    status: 500,
  ...
  await expect(page.locator('[data-component="feedback-service-title"]')).toHaveText("서비스 기본정보");
