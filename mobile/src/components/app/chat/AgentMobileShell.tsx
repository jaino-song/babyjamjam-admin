...
export function AgentMobileShell() {
    ...
    const { messages, status, errorState, sendMessage, stop, resetBranch, sessions, selectSession, deleteSession, approveAction, rejectAction, submitStructuredForm, submitFeedback, task, taskNeedsReconciliation, refreshTask, commandTask } = useAgentChat();
    ...
    return <main data-component="mobile_chat_agent-shell" data-slot="agent-mobile-shell" className="fixed inset-0 flex flex-col bg-background">
        <div ref={contentRef} data-slot="application-content" className="flex min-h-0 flex-1 flex-col">
            ...
            <div data-component="mobile_chat_agent-shell_thread" className="min-h-0 flex-1 overflow-y-auto p-4 pb-6"><div className="flex flex-col gap-4">{messages.map((message) => <article key={message.id} data-component={`mobile_chat_agent-shell_thread_message-${message.role}`} className={message.role === "user" ? "ml-8 rounded-2xl bg-primary p-3 text-primary-foreground" : "mr-8 rounded-2xl border bg-card p-3"}>{message.parts.map((part, index) => <MobileAgentPartRegistry key={`${message.id}-${index}`} data-component={`mobile_chat_agent-shell_thread_message-${message.role}_part-${index}${part.type === "data-action-proposal" ? "_action-approval" : ""}`} part={part as never} task={task} terminalActionIds={terminalActionIds} onEntitySelect={(id, entityType) => void sendMessage(`선택한 엔티티 유형: ${entityType}, 선택한 엔티티 ID: ${id}`)} onTaskEntitySelect={(taskId, choiceSetRef, optionId) => void commandTask(taskId, { command: "select-target", choiceSetRef, optionId })} onApproveAction={(actionId, expectedRevision, acknowledgementToken) => void approveAction(actionId, expectedRevision, acknowledgementToken)} onRejectAction={(actionId) => void rejectAction(actionId)} onSubmitForm={(formId, values) => void submitStructuredForm(formId, values)} />)}{message.role === "assistant" && <div data-slot="feedback" className="mt-3 flex gap-2"><Button type="button" size="sm" variant="ghost" className="min-h-11" onClick={() => void submitFeedback(message.id, "positive")}>좋아요</Button><Button type="button" size="sm" variant="ghost" className="min-h-11" onClick={() => void submitFeedback(message.id, "negative")}>아쉬워요</Button></div>}</article>)}{taskNeedsReconciliation && <section data-component="mobile_chat_agent-shell_thread_task-conflict" data-slot="task-conflict" role="alert" className="rounded-xl border border-destructive/40 p-3"><p className="font-medium">{errorState?.code === "task_conflict" ? errorState.message : "초안이 다른 화면에서 변경되었습니다."}</p><p className="mt-1 text-xs text-muted-foreground">최신 초안을 확인한 뒤 다시 수정해 주세요.</p><Button type="button" variant="outline" className="mt-2 min-h-11" onClick={() => void refreshTask()}>최신 초안 불러오기</Button></section>}{errorState && !taskNeedsReconciliation && <section data-slot="client-error" role="alert" className="rounded-xl border border-destructive/40 p-3"><p className="font-medium">{errorState.message}</p><p className="mt-1 text-xs text-muted-foreground">{errorState.effectState === "nothing-happened" ? "작업은 실행되지 않았습니다." : errorState.effectState === "partial" ? "일부 단계만 완료되었을 수 있습니다. 다시 실행하지 마세요." : "결과 확인 전에는 다시 실행하지 마세요."}</p></section>}</div></div>
