...
function BarSeries({ values, labels, color = "bg-v3-primary" }: { values: readonly number[]; labels?: readonly string[]; color?: string }): ReactElement {
  const max = Math.max(1, ...values);
  return <div data-component="mobile_stats_chart_bars" className="flex min-h-[116px] flex-1 items-end gap-1.5 overflow-hidden rounded-[calc(12px*var(--glint-ui-scale,1))] bg-white px-2 py-3">{values.length === 0 ? <span className="m-auto text-[calc(0.68rem*var(--glint-ui-scale,1))] text-v3-text-muted">데이터가 없습니다.</span> : values.map((value, index) => <span key={`${labels?.[index] ?? index}`} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-1"><span className={`w-full max-w-4 rounded-t ${color}`} style={{ height: `${Math.max(4, (value / max) * 76)}px` }} title={labels?.[index] ?? undefined} /><small className="truncate text-[calc(0.55rem*var(--glint-ui-scale,1))] text-v3-text-muted">{labels?.[index] ?? ""}</small></span>)}</div>;
}

function ErrorReport({ data }: { data: StatsViewData["errors"] }): ReactElement {
  const summary: ErrorSummary = data.summary;
  return <ReportCard dataComponent={`${DETAIL_BASE}_errors_report`} title="Sentry 오류 모니터링"><MetricRows dataComponent={`${DETAIL_BASE}_errors_metrics`} rows={[{ id: "open", label: "미해결", value: `${formatNumber(summary.openCount)}건` }, { id: "new", label: "24시간 신규", value: `${formatNumber(summary.newIn24h)}건` }, { id: "events", label: "7일 발생", value: `${formatNumber(summary.totalEvents7d)}건` }, { id: "users", label: "영향 사용자", value: `${formatNumber(summary.affectedUsers)}명` }, { id: "last", label: "최근 오류", value: relativeTime(summary.lastErrorAt) }]} /><BarSeries values={summary.sparkline7d} color="bg-red-500" /><InfoCard data-component={`${DETAIL_BASE}_errors_severity`} title="심각도"><InfoRow label="Critical" value={summary.severity.critical} /><InfoRow label="Error" value={summary.severity.error} /><InfoRow label="Warning" value={summary.severity.warning} /></InfoCard>{data.issues.length > 0 ? <InfoCard data-component={`${DETAIL_BASE}_errors_issues`} title="주요 이슈">{data.issues.slice(0, 5).map((issue) => <InfoRow key={issue.id} label={issue.title} value={`${formatNumber(issue.count)}건`} />)}</InfoCard> : <p className="rounded-[calc(12px*var(--glint-ui-scale,1))] bg-white p-3 text-[calc(0.68rem*var(--glint-ui-scale,1))] text-v3-text-muted">미해결 이슈가 없습니다.</p>}</ReportCard>;
}

function InquiryReport({ data }: { data: StatsViewData["inquiries"] }): ReactElement {
  const summary: InquirySummary = data.summary;
  return <ReportCard dataComponent={`${DETAIL_BASE}_inquiries_report`} title="PostHog 상담 문의"><MetricRows dataComponent={`${DETAIL_BASE}_inquiries_metrics`} rows={[{ id: "today", label: "오늘", value: `${formatNumber(summary.today)}건` }, { id: "yesterday", label: "어제", value: `${formatNumber(summary.yesterday)}건` }, { id: "week", label: "7일 합계", value: `${formatNumber(summary.sevenDayTotal)}건` }, { id: "conversion", label: "전환율", value: formatConversionRate(summary.conversionRate) }, { id: "last", label: "최근 문의", value: relativeTime(summary.lastSubmissionAt) }]} />{summary.conversionRate === null ? <p data-component={`${DETAIL_BASE}_inquiries_conversion-unavailable`} className="rounded-[calc(12px*var(--glint-ui-scale,1))] bg-white p-3 text-[calc(0.68rem*var(--glint-ui-scale,1))] leading-relaxed text-v3-text-muted">가격 페이지 조회 이벤트에 지점 정보가 없어 지점별 전환율을 집계할 수 없습니다.</p> : null}<BarSeries values={data.daily.map((point) => point.count)} labels={data.daily.map((point) => point.day.slice(5))} color="bg-emerald-500" /><InfoCard data-component={`${DETAIL_BASE}_inquiries_hourly`} title="오늘 시간대">{data.hourly.filter((point) => point.count > 0).slice(0, 8).map((point) => <InfoRow key={point.hour} label={`${String(point.hour).padStart(2, "0")}시`} value={`${formatNumber(point.count)}건`} />)}{data.hourly.every((point) => point.count === 0) ? <p className="text-[0.7rem] text-v3-text-muted">오늘 문의가 없습니다.</p> : null}</InfoCard>{data.recent.length > 0 ? <InfoCard data-component={`${DETAIL_BASE}_inquiries_recent`} title="최근 문의">{data.recent.slice(0, 5).map((item) => <InfoRow key={`${item.distinctId}-${item.timestamp}`} label={item.branchSlug ?? "지점 미상"} value={relativeTime(item.timestamp)} />)}</InfoCard> : null}</ReportCard>;
}
...
function TrafficReport({ data }: { data: StatsViewData["traffic"] }): ReactElement {
  ...
  return <ReportCard dataComponent={`${DETAIL_BASE}_traffic_report`} title="PostHog 사이트 트래픽"><MetricRows dataComponent={`${DETAIL_BASE}_traffic_metrics`} rows={[{ id: "pv", label: "오늘 페이지뷰", value: formatNumber(summary.today.pv) }, { id: "unique", label: "오늘 방문자", value: formatNumber(summary.today.unique) }, { id: "week", label: "7일 페이지뷰", value: formatNumber(summary.sevenDayTotal.pv) }, { id: "session", label: "평균 세션", value: `${Math.round(summary.avgSessionSeconds)}초` }, { id: "bounce", label: "이탈률", value: formatPercent(summary.bounceRate) }]} /><BarSeries values={data.trend.map((point) => point.pv)} labels={data.trend.map((point) => point.day.slice(5))} /><InfoCard data-component={`${DETAIL_BASE}_traffic_top-pages`} title="인기 페이지">{data.topPages.slice(0, 8).map((page) => <InfoRow key={page.path} label={page.path} value={`${formatNumber(page.pv)} PV`} />)}</InfoCard><InfoCard data-component={`${DETAIL_BASE}_traffic_devices`} title="디바이스">{data.devices.slice(0, 5).map((device) => <InfoRow key={device.label} label={device.label} value={formatPercent(device.pct)} />)}</InfoCard><InfoCard data-component={`${DETAIL_BASE}_traffic_sources`} title="유입 경로">{data.sources.slice(0, 5).map((source) => <InfoRow key={source.label} label={source.label} value={formatPercent(source.pct)} />)}</InfoCard></ReportCard>;
}
