import { getQueryClient, shouldRetryQuery } from "./queryClient";
...
describe("query client defaults", () => {
    it("wires the status-aware retry policy into the effective QueryClient", () => {
        const retry = getQueryClient().getDefaultOptions().queries?.retry;

        expect(retry).toBe(shouldRetryQuery);
        if (typeof retry !== "function") {
            throw new Error("QueryClient retry policy is not a function");
        }
