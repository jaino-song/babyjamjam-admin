...
function contractBirthdayParts(raw: string): [string, string, string] | null {
    ...
    if (separated) {
        if (separated[5] && separated[2] !== ".") return null;
...
export function normalizeContractBirthday(raw: string | null | undefined, now: Date = new Date()): string | null {
    ...
    if (year < MIN_CONTRACT_BIRTH_YEAR || !isValidCalendarDate(year, month, day)) return null;
