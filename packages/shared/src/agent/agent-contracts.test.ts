...
describe("agent contracts", () => {
    ...
    it("preserves safe optional form input metadata through form parsing", () => {
        ...
        expect(parsed.fields).toEqual([expect.objectContaining({
            name: "birthday",
            inputMode: "numeric",
            placeholder: "YYMMDD",
            maxLength: 6,
    ...
    it("rejects unsafe form input metadata and omits absent optional attributes", () => {
        const field = { name: "name", label: "이름", type: "text" as const };

        expect(AgentFormFieldSchema.safeParse({ ...field, inputMode: "script" }).success).toBe(false);
        expect(AgentFormFieldSchema.safeParse({ ...field, placeholder: "x".repeat(201) }).success).toBe(false);
        expect(AgentFormFieldSchema.safeParse({ ...field, maxLength: 0 }).success).toBe(false);
        expect(AgentFormFieldSchema.safeParse({ ...field, maxLength: 1001 }).success).toBe(false);
        expect(AgentFormFieldSchema.parse(field)).toEqual(field);
    });
});
