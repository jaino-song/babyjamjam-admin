...
const UNKNOWN_MESSAGES: Readonly<Record<ErrorLocale, string>> = Object.freeze({
    "ko-KR": "요청을 처리하지 못했어요. 정확한 원인은 확인되지 않았어요.",
    "en-US": "The request could not be completed. The exact cause is unknown.",
});

const READ_FAILURE_MESSAGES: Readonly<Record<ErrorLocale, string>> = Object.freeze({
    "ko-KR": "조회 결과를 확인하지 못했어요. 잠시 후 상태를 확인해 주세요.",
    "en-US": "The read result could not be confirmed. Check the status again later.",
});

const MUTATION_FAILURE_MESSAGES: Readonly<Record<ErrorLocale, string>> = Object.freeze({
    "ko-KR": "변경 결과를 확인하지 못했어요. 작업 상태를 확인해 주세요.",
    "en-US": "The change result could not be confirmed. Check the operation status.",
});
...
> = {
    REQUEST_INVALID: {
        ...
        detail: {
            "ko-KR": "요청을 확인한 뒤 다시 보내 주세요.",
            "en-US": "Check the request and send it again.",
        },
    },
    VALIDATION_FAILED: {
        status: 400,
        ...
        detail: {
            "ko-KR": "입력값을 확인한 뒤 다시 보내 주세요.",
            "en-US": "Check the input values and send the request again.",
        },
    },
    AUTH_REQUIRED: {
        status: 401,
    ...
    ACCESS_DENIED: {
        ...
        detail: {
            "ko-KR": "이 작업을 수행할 권한이 있는지 확인해 주세요.",
            "en-US": "Check that you have permission to perform this operation.",
        },
    },
    RESOURCE_NOT_FOUND: {
        status: 404,
    ...
    REQUEST_CONFLICT: {
        ...
        detail: {
            "ko-KR": "현재 상태를 확인한 뒤 다시 요청해 주세요.",
            "en-US": "Check the current state before sending the request again.",
        },
    },
    METHOD_NOT_ALLOWED: {
        status: 405,
    ...
    INTERNAL_ERROR: {
        ...
        detail: {
            "ko-KR": "서버 오류로 요청을 처리하지 못했어요.",
            "en-US": "The server could not complete the request.",
        },
    },
    DEPENDENCY_UNAVAILABLE: {
        status: 503,
        ...
        detail: {
            "ko-KR": "연결된 서비스의 상태를 확인해 주세요.",
            "en-US": "The connected service is unavailable; check its status.",
        },
    },
    UPSTREAM_INVALID_RESPONSE: {
        status: 502,
    ...
    UPSTREAM_TIMEOUT: {
        ...
        detail: {
            "ko-KR": "연결된 서비스의 처리 결과를 확인해 주세요.",
            "en-US": "Check the connected service for the processing result.",
        },
    },
    CONTRACT_ALREADY_SIGNED: {
        status: 409,
...
function problemCodeFromUnknown(value: unknown): ProblemCode | undefined {
    ...
    const directCode = readOwn(value, "code").value;
    if (isProblemCode(directCode)) {
        return directCode;
    }
    const nestedProblem = readOwn(value, "problem").value;
    if (isRecord(nestedProblem) && isProblemCode(readOwn(nestedProblem, "code").value)) {
        return readOwn(nestedProblem, "code").value as ProblemCode;
    }
    return undefined;
}
