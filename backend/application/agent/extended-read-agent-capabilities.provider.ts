...
const QuerySchema = z.object({
    query: z.string().trim().max(100).optional().describe(
        "Optional keyword filter. Only applies to capabilities that expose text search over specific fields — see this capability's description for which fields; if none are listed, this filter has no effect."
    ),
    limit: z.number().int().positive().max(50).optional().describe("Maximum number of rows to return (default 20, max 50)."),
});
...
export class ExtendedReadAgentCapabilitiesProvider implements AgentCapabilityProviderContract {
    ...
    getCapabilities(): CapabilityDefinition[] {
        return [
            this.read("consultations.list", "consultations", "List consultation inquiries (pre-registration leads) for the current branch, newest first. Use for: 상담 문의 목록, 전체 문의 확인, 최근 문의. Input: optional query — when given, matched partially, case-insensitively, against the mother's name, status, or preferred caregiver name; omit it to list everything. Optional limit (max 50, default 20). Returns: id, motherName, dueDate, voucherType, preferredCaregiverName, referralSource, source, status, readAt, createdAt, updatedAt.", "consultation_inquiry", {}, { select: { id: true, motherName: true, dueDate: true, voucherType: true, preferredCaregiverName: true, referralSource: true, source: true, status: true, readAt: true, createdAt: true, updatedAt: true }, searchFields: ["motherName", "status", "preferredCaregiverName"] }),
            this.read("consultations.search", "consultations", "Search consultation inquiries (pre-registration leads) for the current branch by a specific keyword — behaves like consultations.list but is meant for when you already have a name, status, or caregiver to filter by. Use for: 특정 산모 상담 찾기, 상담 상태로 검색, 담당 관리사로 검색. Input: query, matched partially, case-insensitively, against the mother's name, status, or preferred caregiver name. Returns the same fields as consultations.list. Requires a specific value; a generic word such as 산모 or 고객 will match too broadly — use consultations.list without a query to browse everything instead.", "consultation_inquiry", {}, { select: { id: true, motherName: true, dueDate: true, voucherType: true, preferredCaregiverName: true, referralSource: true, source: true, status: true, readAt: true, createdAt: true, updatedAt: true }, searchFields: ["motherName", "status", "preferredCaregiverName"] }),
            this.read("consultations.unread", "consultations", "List only unread consultation inquiries for the current branch, newest first. Use for: 안읽은 상담, 미확인 문의, 새 문의 확인. Input: optional query (same match fields as consultations.list: mother's name, status, or preferred caregiver name) and optional limit (max 50, default 20). Returns the same fields as consultations.list; every row has readAt = null.", "consultation_inquiry", { readAt: null }, { select: { id: true, motherName: true, dueDate: true, voucherType: true, preferredCaregiverName: true, referralSource: true, source: true, status: true, readAt: true, createdAt: true, updatedAt: true }, searchFields: ["motherName", "status", "preferredCaregiverName"] }),
            ...
            this.read("files.search", "files", "Search authorized files for the current branch", "document", {}, { select: { id: true, name: true, description: true, tags: true, mimeType: true, fileSize: true, orgId: true, uploadedBy: true, visibilityScope: true, createdAt: true, updatedAt: true, categoryId: true, branchId: true }, searchFields: ["name", "description"] }),
            ...
            this.read("service-records.oversight", "service-records", "Read service-record case oversight rows (session-count and finalization/document tracking) for the current branch — operational status, not the submitted form content. Use for: 제공기록지 진행상황, 서비스 종료 처리 현황, 최종 승인 여부. Input: optional limit (max 50, default 20); any query text is ignored — this capability has no search field. Resolve a client's id via clients.search first, then match it against the returned clientId. Returns: id, clientId, status, startDate, endDate, requiredSessionCount, formVersion, plus finalization/completion timestamps and retry metadata.", "service_record_case", {}, { select: { id: true, clientId: true, status: true, startDate: true, endDate: true, requiredSessionCount: true, formVersion: true, completedAt: true, finalizationDueAt: true, finalizationStartedAt: true, finalizedAt: true, documentsCompletedAt: true, finalizationAttempts: true, nextAttemptAt: true, version: true, createdAt: true, updatedAt: true } }),
