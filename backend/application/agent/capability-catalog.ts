...
const reads: CapabilityCatalogEntry[] = ([
    ...
    ["schedules.list", "schedules", "activity", "List employee work schedules (client assignments) for the current branch, optionally filtered to one calendar date. Use for: 오늘 일정, 이번주 스케줄, 방문 일정 확인, 특정 날짜 배정 확인. Input: optional date (YYYY-MM-DD); when given, only schedules overlapping that date are returned, up to 50 rows sorted by start date. Does not accept a client or employee name — cross-reference the returned ids with clients.get or employees.get. Returns: id, clientId, primaryEmployeeId, secondaryEmployeeId, startDate, endDate, replaced."],
