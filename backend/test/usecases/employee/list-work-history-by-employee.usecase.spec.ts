...
describe("ListWorkHistoryByEmployeeUsecase", () => {
    ...
    it("throws the not-found problem body for an employee outside the selected branch", async () => {
        await expect(usecase.execute(branchId, 999, 1, limit)).rejects.toMatchObject({
            status: 404,
            response: {
